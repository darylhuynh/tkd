# Arc 3 — Panel Storyboard (Chapters 21–35)

**First Tournament** · Trauma trigger + team corners + sport education in live matches.

Append Global Style from [STORYBOARD-GUIDE](./STORYBOARD-GUIDE.md). See [OLYMPIC-TKD-EDUCATION.md](../OLYMPIC-TKD-EDUCATION.md).

**Kyorugi gear:** Spar/match/`[ACTION]` panels = white dobok + chest hogu + headgear + forearm + shin/foot guards — see [STORYBOARD-GUIDE § Kyorugi Sparring Gear](./STORYBOARD-GUIDE.md#kyorugi-sparring-gear-required).

**Premier flash (extended):** [flash-premier-board-break.md](../scripts/flash-premier-board-break.md) → **§28.1–28.8** · [flash-premier-knockout-bounty.md](../scripts/flash-premier-knockout-bounty.md) § Scene 3 → **22.4**  
**Short thread:** [short-yuna-pineapple-fanta.md](../scripts/short-yuna-pineapple-fanta.md) → **§34.4–34.10**

---

## Chapter 21 — Equipment Fitting [TEACH + CARD]

### 21.0 — Team Roster Cards [CARD]
**Scene:** Montage — six quick stat cards (stacked vertical scroll) — Kwon's roster debut at tournament.
**Cards:** Ethan, TJ, Kieryn, Kian, Ariana, Logan — see [CHARACTER-CARDS.md](../storyboards/CHARACTER-CARDS.md)
**Prompt:** `stacked fighter profile cards montage six taekwondo athletes in full kyorugi gear empty stat boxes vertical scroll, STYLE: CARD TOURNAMENT`

### 21.1 — Gear Table [TEACH]
**Scene:** MV lays out electronic hogus, headgear, arm/leg guards. Labels each piece.
**Dialogue:** MV: "Chest sensor — two for body. Head sensor — three for head. Gear only scores clean contact."
**Prompt:** `taekwondo electronic scoring gear laid out labeled chest head guards instructional, STYLE: KWONS TEACH`

### 21.2 — Team Helps Strap [HAIKYUU]
**Scene:** Logan can't figure leg strap — Kieryn sighs, fixes it. TJ helps Ariana headgear.
**Dialogue:** Kieryn: "Left strap. Right strap. Brain."
**Prompt:** `teammates helping each other strap taekwondo gear comedy teamwork, STYLE: HAIKYUU`

### 21.3 — Ethan Mirror [TEACH]
**Scene:** Ethan **fully geared** — white dobok + electronic chest hogu + headgear + forearm guards + shin/foot guards — mirror shot. Silver belt hidden under hogu.
**Dialogue:** Ethan (internal): *Armor on. Still the same kid underneath.*
**Prompt:** `teen full olympic kyorugi kit mirror white dobok chest hogu headgear forearm shin foot guards, STYLE: TOURNAMENT`

### 21.4 — Bracket Mismatch [TEACH]
**Scene:** Bracket board — Ethan's name under heavier division line.
**Dialogue:** MV: "Weight class got bumped. You'll fight up. Happens in opens."
**Prompt:** `tournament bracket board weight class mismatch highlighted, coach explaining, STYLE: TOURNAMENT TEACH`

### 21.5 — Team Pep [HAIKYUU]
**Scene:** Hands in — Logan too short, stands on tiptoes.
**Dialogue:** TJ: "We corner you. You don't fight alone."
**Prompt:** `team gear huddle hands stacked tiptoe comedy, STYLE: HAIKYUU`

### 21.6 — Sensor Calibration [TEACH]
**Scene:** MV taps each fighter's chest hogu — LED test blink.
**Dialogue:** MV: "If it doesn't flash on contact, tell me before you bleed for nothing."
**Prompt:** `coach testing electronic chest sensor LED blink taekwondo gear, STYLE: KWONS TEACH`

### 21.7 — Ariana & Kian Arms [HAIKYUU]
**Scene:** Ariana threads Kian's forearm guard strap — he's impatient.
**Dialogue:** Ariana: "Hold still." · Kian: "I'm holding." · Ariana: "You're vibrating."
**Prompt:** `girl helping boy strap forearm guard comedy patience, STYLE: HAIKYUU`

### 21.8 — Periphery Test [TEACH]
**Scene:** Logan spins in headgear — walks into wall.
**Dialogue:** Kieryn: "You can't see your feet. That's the point." · Logan: "I can see the wall."
**Prompt:** `teen in taekwondo headgear limited vision walking into wall comedy, STYLE: TEACH HAIKYUU`

### 21.9 — Ethan Sensor Tap [TEACH]
**Scene:** MV light tap to Ethan's hogu — clean flash. Ethan flinches anyway.
**Dialogue:** MV: "Clean contact. That's two." · Ethan: "…Yeah."
**Prompt:** `electronic hogu sensor flash clean tap teen flinching, STYLE: KWONS TEACH`

### 21.10 — Kieryn Shadow Trash Talk [HAIKYUU]
**Scene:** Kieryn faces empty ring — mutters at imaginary opponent.
**Dialogue:** Kieryn: "Try me. I dare you." · TJ: "She's been doing that since six AM."
**Prompt:** `teen girl shadow trash talk empty tournament ring, STYLE: HAIKYUU`

### 21.11 — Logan Mirror Selfie [HAIKYUU]
**Scene:** Logan discovers venue mirror — full gear — peace sign.
**Dialogue:** Logan: "Document the armor!" · MV: "Put the phone away."
**Prompt:** `short teen full kyorugi gear mirror peace sign comedy, STYLE: HAIKYUU`

### 21.12 — Belt Check [KWONS]
**Scene:** MV walks the line — silver belts under hogus — Logan's crooked.
**Dialogue:** MV: "Belts straight. You represent the school before you throw a kick."
**Prompt:** `coach inspecting team belt alignment pre tournament, STYLE: KWONS`

### 21.13 — Bracket Close-Up [TEACH]
**Scene:** Board extreme close — **Derek Cole · Apex TKD · Open −54 kg**.
**Dialogue:** Ethan (internal): *Taller. Heavier. Open division.*
**Prompt:** `tournament bracket close up opponent name school logo, STYLE: TOURNAMENT TEACH`

### 21.14 — Weight Line [TEACH]
**Scene:** Diagram inset — Ethan's division bumped above his weigh-in class.
**Dialogue:** MV: "Opens merge light juniors with heavy cadets sometimes. You still score the same."
**Prompt:** `weight class diagram bumped division educational inset, STYLE: TEACH TOURNAMENT`

### 21.15 — Team Photo Attempt [HAIKYUU]
**Scene:** Ariana holds phone — Logan blinks — Kian photobombs.
**Dialogue:** Kieryn: "Again." · Logan: "I wasn't ready!" · Kieryn: "You never are."
**Prompt:** `team group photo attempt full gear someone blinking comedy, STYLE: HAIKYUU`

### 21.16 — Kian Nervous Shadow [ACTION]
**Scene:** Kian shadowboxes in corner — fast feet — can't sit still.
**Dialogue:** Kian (internal): *Move move move—*
**Prompt:** `restless teen shadowboxing corner pre fight nervous energy, STYLE: KWONS ACTION`

### 21.17 — TJ Emperor Stance [TEACH]
**Scene:** TJ alone — wide base — slow chamber — team watches without interrupting.
**Dialogue:** MV (quiet): "That's the one."
**Prompt:** `muscular teen perfect taekwondo chamber stance teammates watching, STYLE: KWONS TEACH`

### 21.18 — Ariana Bandages [TEACH]
**Scene:** Ariana wraps wrist under forearm guard — methodical — MV nods once.
**Dialogue:** Ariana (internal): *Smart. Not broken.*
**Prompt:** `teen girl wrapping wrist bandages under forearm guard controlled, STYLE: KWONS TEACH`

### 21.19 — Walk to Registration [HAIKYUU]
**Scene:** Wide — full team in gear bags — arena hallway — first tournament scale.
**Dialogue:** Logan: "This hallway is bigger than our whole dojang." · TJ: "Good."
**Prompt:** `taekwondo team walking tournament hallway wide shot scale, STYLE: HAIKYUU TOURNAMENT`

---

## Chapter 22 — Registration & Venue

### 22.1 — Weigh-In Chaos
**Scene:** Scale line — Kieryn trash talk at rival cadet.
**Dialogue:** Kieryn: "Don't breathe out. You'll miss weight and cry."
**Prompt:** `taekwondo weigh in line girl trash talk, tournament venue, STYLE: TOURNAMENT`

### 22.2 — Logan Waves [HAIKYUU]
**Scene:** Logan waves at every school banner like parade.
**Dialogue:** Logan: "Hi! We came to win! Or try!"
**Prompt:** `cheerful teen waving at tournament crowd multiple school banners, STYLE: HAIKYUU`

### 22.3 — Venue Scale [TEACH]
**Scene:** Wide — multiple rings, electronic scoreboards overhead.
**Dialogue:** MV: "Ring four is ours. Watch the board — it tells you what's real."
**Prompt:** `large taekwondo tournament hall multiple rings electronic scoreboards wide shot, STYLE: TOURNAMENT TEACH`

### 22.4 — No Knockout Bounty [REAL #28 contrast]
**Scene:** MV team huddle — safety briefing before matches.
**Dialogue:** MV: "We win on **points**. We win on **control**. We do **not** chase knockouts. Black belt means **discipline**, not cruelty." · Logan: *(whisper)* "Was that… a thing somewhere?" · Ethan: "Thousand won. If you knock them out. He said I was **too nice** to be one." · Kieryn: "That's not taekwondo. That's a lawsuit." · MV: "Head kicks are three points — not invitations to hurt someone for show."
**Script:** [flash-premier-knockout-bounty.md](../scripts/flash-premier-knockout-bounty.md) § Scene 3
**Prompt:** `coach team huddle black belt discipline not cruelty tournament briefing contrast flashback trauma, STYLE: KWONS HAIKYUU TEACH`

### 22.5 — Registration Line [HAIKYUU]
**Scene:** Long desk queue — Kieryn fills forms for half the team.
**Dialogue:** Kieryn: "Spell your legal name. Not your gamertag." · Kian: "…Fine."
**Prompt:** `tournament registration desk line teens filling forms comedy, STYLE: HAIKYUU TOURNAMENT`

### 22.6 — Banner Hall [TEACH]
**Scene:** Wide pan — dozens of school banners — regional colors.
**Dialogue:** MV: "Every banner is someone who thinks they're ready. So are we."
**Prompt:** `tournament hall school banners wide pan multiple martial arts schools, STYLE: TOURNAMENT TEACH`

### 22.7 — Premier Banner [PREMIER inset]
**Scene:** **Premier TKD** banner on far wall — Ethan freezes half a step.
**Dialogue:** Ethan (internal): *Don't.*
**Prompt:** `premier taekwondo school banner far wall teen freezing glance, STYLE: PREMIER inset TOURNAMENT`

### 22.8 — Kieryn Makes Weight [HAIKYUU]
**Scene:** Scale — Kieryn steps off — fist pump micro.
**Dialogue:** Kieryn: "Cadet minus forty-nine. Obviously."
**Prompt:** `teen girl making weight scale smirk confident, STYLE: HAIKYUU TOURNAMENT`

### 22.9 — Logan Scale Comedy [HAIKYUU]
**Scene:** Logan on tiptoes on scale — MV deadpan.
**Dialogue:** MV: "Both feet flat." · Logan: "I'm trying!"
**Prompt:** `short teen tiptoe on weigh in scale comedy coach deadpan, STYLE: HAIKYUU`

### 22.10 — Coach Credentials [TEACH]
**Scene:** MV lanyard scanned — official wristband — assistant points ring four.
**Dialogue:** MV: "Corner access. I stay where the rules let me."
**Prompt:** `taekwondo coach tournament credentials wristband ring assignment, STYLE: KWONS TEACH`

### 22.11 — Ring Assignment [TEACH]
**Scene:** Board — **Ring 4 — Kwon's Taekwondo** — match times listed.
**Dialogue:** TJ: "Ethan first. Then me. Then chaos."
**Prompt:** `tournament ring assignment board school name highlighted, STYLE: TOURNAMENT TEACH`

### 22.12 — Derek on Schedule [TEACH]
**Scene:** Ethan finds Derek's name on same ring block — earlier slot.
**Dialogue:** Ethan (internal): *Of course.*
**Prompt:** `teen reading match schedule opponent name same ring, STYLE: TOURNAMENT`

### 22.13 — PA Announcement [HAIKYUU]
**Scene:** Overhead speaker — muffled — Logan salutes the ceiling.
**Dialogue:** PA: "Athletes report to holding—" · Logan: "Yes sir."
**Prompt:** `tournament PA speaker teen saluting ceiling comedy, STYLE: HAIKYUU`

### 22.14 — Warmup Zone [HAIKYUU]
**Scene:** Team stretches — synchronized accidentally — then not.
**Dialogue:** Kian: "Left." · Ariana: "Your other left."
**Prompt:** `team stretching warmup zone comedy wrong direction, STYLE: HAIKYUU TOURNAMENT`

### 22.15 — Apex Stare [TOURNAMENT]
**Scene:** Apex TKD athletes pass — Derek doesn't look — teammates do.
**Dialogue:** Kieryn: "Let them look. We came to fight."
**Prompt:** `rival school athletes walking past cold stare tournament, STYLE: TOURNAMENT`

### 22.16 — Scoreboard Colors [TEACH]
**Scene:** MV points at live board — blue/red — point flash demo on loop.
**Dialogue:** MV: "Blue corner. Red corner. Board never lies. Ref might."
**Prompt:** `electronic taekwondo scoreboard color corners educational, STYLE: TOURNAMENT TEACH`

### 22.17 — Kian Fire [HAIKYUU]
**Scene:** Kian cracks neck — grin feral — Logan copies badly.
**Dialogue:** Kian: "Three hours in a van. I'm done sitting." · Logan: "Same!"
**Prompt:** `restless teens pre fight energy neck crack comedy, STYLE: HAIKYUU`

### 22.18 — Overpriced Snack [HAIKYUU]
**Scene:** Logan holds tiny protein bar — price sticker visible — shock.
**Dialogue:** Logan: "Eight dollars." · Kieryn: "Put it back." · Logan: "…Half."
**Prompt:** `teen shocked at overpriced tournament snack bar comedy, STYLE: HAIKYUU`

### 22.19 — Hall Wide — Chapter End [TOURNAMENT]
**Scene:** Extreme wide — four rings active — Kwon's team small in frame — scale established.
**Dialogue:** *(none — chapter end wide)*
**Prompt:** `massive taekwondo tournament hall four rings active team small in frame, STYLE: TOURNAMENT`

### 22.20 — MV Last Look [KWONS]
**Scene:** MV watches Ethan walk toward holding — doesn't call him back.
**Dialogue:** MV (internal): *Different corner. Different rules.*
**Prompt:** `coach watching teen walk to holding area quiet trust, STYLE: KWONS TOURNAMENT`

---

## Chapter 23 — Travel Banter [HAIKYUU]

### 23.1 — Bus / Car Convoy
**Scene:** Team van — Kieryn earbuds, Kian arm wrestle Logan, Ethan window seat headphones.
**Prompt:** `team van interior diverse teens pre tournament energy, STYLE: HAIKYUU`

### 23.2 — Baseline Banter Callback
**Dialogue:** TJ: "Stop fighting." · Kian: "I was." · Logan: "Can we stop for food?"
**Prompt:** `team van playful argument, STYLE: HAIKYUU`

### 23.3 — MV Tap [HAIKYUU]
**Scene:** MV taps Ethan's shoulder — Ethan removes headphones one ear.
**Dialogue:** MV: "When you're ready — not before."
**Prompt:** `coach gentle shoulder tap teen removing headphones, quiet trust moment, STYLE: KWONS`

### 23.4 — Highway Wide [HAIKYUU]
**Scene:** Van on highway — tournament flyer taped to window — sun rising.
**Dialogue:** Logan: "Are we there yet?" · Everyone: "No."
**Prompt:** `team van highway sunrise tournament flyer window comedy, STYLE: HAIKYUU`

### 23.5 — Snack Politics [HAIKYUU]
**Scene:** TJ divides chips — Logan gets crumbs — accepts happily.
**Dialogue:** TJ: "Fair is fair." · Logan: "Crumbs are still chips."
**Prompt:** `teens sharing snacks van chip bag comedy fairness, STYLE: HAIKYUU`

### 23.6 — Kieryn Playlist [HAIKYUU]
**Scene:** Kieryn one earbud out — offers other to Ariana — declined — smirk.
**Dialogue:** Kieryn: "Your loss." · Ariana: "I prefer silence."
**Prompt:** `teen girl earbuds offer declined van banter, STYLE: HAIKYUU`

### 23.7 — Kian vs Logan Arm Wrestle [HAIKYUU]
**Scene:** Arm wrestle on gear bag — Logan surprisingly stubborn — Kian wins slow.
**Dialogue:** Kian: "You're annoying strong for a gremlin." · Logan: "Thanks!"
**Prompt:** `arm wrestle on gear bag van comedy underdog stubborn, STYLE: HAIKYUU`

### 23.8 — Ethan Window [KWONS]
**Scene:** Ethan forehead on glass — reflection double — headphones on again.
**Dialogue:** Ethan (internal): *Same highway. Different destination.*
**Prompt:** `teen forehead on van window reflection quiet headphones, STYLE: KWONS`

### 23.9 — MV Rearview [KWONS]
**Scene:** MV eyes in rearview — sees whole team — small smile — says nothing.
**Dialogue:** *(none)*
**Prompt:** `coach rearview mirror seeing team van quiet pride, STYLE: KWONS`

### 23.10 — Rest Stop Sprint [HAIKYUU]
**Scene:** Logan races to bathroom — Kian chases — MV doesn't stop them.
**Dialogue:** TJ: "Let them burn it off." · Ariana: "Wise."
**Prompt:** `teens sprinting rest stop bathroom race comedy, STYLE: HAIKYUU`

### 23.11 — Baseline Callback II [HAIKYUU]
**Scene:** Van restarted — same argument pattern as Arc 2.
**Dialogue:** Kian: "I wasn't fighting." · TJ: "You were." · Logan: "Food now?"
**Prompt:** `team van recurring argument pattern comedy callback, STYLE: HAIKYUU`

### 23.12 — Gear Check Mid-Trip [TEACH]
**Scene:** MV calls roll — gear present — Logan missing one shin guard — panic — found.
**Dialogue:** Logan: "It was under the seat!" · Kieryn: "Under YOUR seat. Shocking."
**Prompt:** `missing shin guard panic van gear check comedy, STYLE: TEACH HAIKYUU`

### 23.13 — Ariana Nap [HAIKYUU]
**Scene:** Ariana asleep sitting perfect posture — Logan draws on her hand — caught.
**Dialogue:** Ariana: *(eyes open)* "Don't."
**Prompt:** `girl sleeping upright van teen caught drawing on hand comedy, STYLE: HAIKYUU`

### 23.14 — TJ Captain Count [HAIKYUU]
**Scene:** TJ counts heads before re-entry — six — plus MV — seven.
**Dialogue:** TJ: "All accounted for. Let's go lose our minds properly."
**Prompt:** `muscular teen counting team heads van captain energy, STYLE: HAIKYUU`

### 23.15 — Ethan One Ear Off [KWONS]
**Scene:** Ethan removes headphones — both ears — without MV asking this time.
**Dialogue:** Ethan: "I'm ready." · MV: "Good."
**Prompt:** `teen removing headphones both ears self initiated readiness, STYLE: KWONS`

### 23.16 — Venue First Sight [TOURNAMENT]
**Scene:** Van rounds corner — arena exterior — team goes quiet.
**Dialogue:** Kian: "…That's big."
**Prompt:** `team van first sight tournament arena exterior awe, STYLE: TOURNAMENT`

### 23.17 — Parking Lot Chaos [HAIKYUU]
**Scene:** Cars everywhere — Logan waves at strangers again — Kieryn pulls him.
**Dialogue:** Kieryn: "We don't know them." · Logan: "We might after today!"
**Prompt:** `tournament parking lot chaos teen waving strangers comedy, STYLE: HAIKYUU`

### 23.18 — Gear Unload [HAIKYUU]
**Scene:** Assembly line — bags handed — Logan drops his — TJ catches.
**Dialogue:** TJ: "Eyes up." · Logan: "Thanks, hyung."
**Prompt:** `team unloading gear bags parking lot teamwork catch, STYLE: HAIKYUU`

### 23.19 — Entrance March [HAIKYUU]
**Scene:** Team walks in formation — accidental — Logan in middle — chin up.
**Dialogue:** Kieryn: "Look like we belong." · Logan: "We do!"
**Prompt:** `taekwondo team walking tournament entrance formation pride, STYLE: HAIKYUU TOURNAMENT`

### 23.20 — Doors Open Wide [TOURNAMENT]
**Scene:** Automatic doors — noise rush — chapter end — Ethan steps through last.
**Dialogue:** Ethan (internal): *I've been here before.*
**Prompt:** `tournament arena doors opening noise rush team entering wide, STYLE: TOURNAMENT`

---

## Chapter 24 — Ethan Gears Up

### 24.1 — Alone in Mirror
**Scene:** **Full kyorugi kit** — Ethan tightens headgear strap — forearm and shin guards visible.
**Dialogue:** Ethan (internal): *I've been here before.*
**Prompt:** `teen alone adjusting taekwondo headgear mirror nervous, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 24.2 — Opponent Scout
**Scene:** **Derek Cole** (Apex TKD) — taller — **full competition gear** — warming up spinning hooks.
**Prompt:** `taller taekwondo fighter Derek Cole warming up spinning kicks background, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 24.3 — MV Corner Lesson [TEACH]
**Scene:** MV points at opponent's tell — hip drops before spin.
**Dialogue:** MV: "Hook kick — watch the chamber. If he spins, turning kick doubles. Cover high."
**Prompt:** `coach pointing out opponent technique tell instructional corner talk, STYLE: TOURNAMENT TEACH`

### 24.4 — Walk to Ring [HAIKYUU]
**Scene:** Team lines path — **Ethan in full kit** — fist bumps. Logan: "Hit him like he owes you ramen!"
**Prompt:** `team fist bump tunnel sending fighter to ring haikyuu sendoff, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU`

### 24.5 — Bracket Board [TEACH]
**Scene:** Ethan stares at board — **Derek Cole · Apex · 2nd Dan · Open −54**.
**Dialogue:** Ethan (internal): *Two inches taller on paper. Lies.*
**Prompt:** `teen reading bracket board opponent stats school logo, STYLE: TOURNAMENT TEACH`

### 24.6 — Arc 1 Flash Rhyme [PREMIER inset]
**Timeline:** **FLASH** — Arc 1 Ch 1 bleachers — **PRESENT** — holding area.
**Scene:** One-panel inset — younger Ethan alone — same empty eyes — snap back.
**Dialogue:** Ethan (internal): *Not there. Here.*
**Prompt:** `flashback inset bleachers alone teen desaturated snap to present, STYLE: PREMIER inset`

### 24.7 — Derek Spin Scout [ACTION]
**Scene:** Through crowd gap — Derek **spinning hook** air — hip drop visible — slow-mo inset.
**Dialogue:** MV (O.S.): "You see that?"
**Prompt:** `opponent spinning hook kick warmup hip drop scout inset, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION TEACH`

### 24.8 — Counter Stance Drill [TEACH]
**Scene:** MV adjusts Ethan's guard — low — chin tucked — feet wider.
**Dialogue:** MV: "Counter fighter doesn't chase. You let them gamble."
**Prompt:** `coach adjusting teen counter fighting stance low guard instructional, STYLE: KWONS TEACH`

### 24.9 — Kieryn Trash Talk Scout [HAIKYUU]
**Scene:** Kieryn watches Derek's corner — lip-reads — smirks.
**Dialogue:** Kieryn: "His corner said spin early. They're not subtle."
**Prompt:** `teen girl scouting opponent corner lip read smirk, STYLE: HAIKYUU`

### 24.10 — Logan Pep Escalation [HAIKYUU]
**Scene:** Logan bounces — too much energy — TJ puts hand on his head.
**Dialogue:** Logan: "You got this!" · TJ: "He knows. Breathe."
**Prompt:** `hyperactive teen pep talk hand on head calm down comedy, STYLE: HAIKYUU`

### 24.11 — Ariana Quiet [KWONS]
**Scene:** Ariana tapes Ethan's glove strap — no speech — firm pull.
**Dialogue:** Ariana: "Guard up. Always."
**Prompt:** `stoic girl taping teammate glove strap quiet support, STYLE: KWONS`

### 24.12 — Kian Fire Redirect [HAIKYUU]
**Scene:** Kian shadow-kicks beside Ethan — not showing off — syncing rhythm.
**Dialogue:** Kian: "First kick's a lie. Second one's the truth."
**Prompt:** `two teens shadow kick rhythm holding area pre fight, STYLE: HAIKYUU ACTION`

### 24.13 — Holding Call [TOURNAMENT]
**Scene:** PA — **Hyun, Ethan — Ring 4** — team moves as one.
**Dialogue:** PA: "Blue corner — Hyun, Ethan."
**Prompt:** `tournament PA call holding area teen standing team moving, STYLE: TOURNAMENT`

### 24.14 — Corridor Walk [HAIKYUU]
**Scene:** Team flanks Ethan — tunnel of teammates — Blue Lock intensity.
**Dialogue:** Kieryn: "Eyes on his hip. Not his face."
**Prompt:** `team flanking fighter corridor walk intense sendoff, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU TOURNAMENT`

### 24.15 — Derek Entrance [ACTION]
**Scene:** Derek enters opposite — taller — bounces — doesn't bow at Ethan yet.
**Dialogue:** Derek: *(nods once)*
**Prompt:** `taller opponent entering ring opposite teen nod cold respect, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 24.16 — Ref Gear Check [TEACH]
**Scene:** Ref inspects sensors — taps hogus — both fighters still.
**Dialogue:** Ref: "Chung. Hong. Bow."
**Prompt:** `referee gear sensor check taekwondo pre match, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH`

### 24.17 — MV Corner Set [KWONS]
**Scene:** MV in corner chair — illegal zone line visible — towel ready.
**Dialogue:** MV: "Cut. Don't chase. Cover high."
**Prompt:** `coach corner chair towel ready tournament rules zone, STYLE: KWONS TOURNAMENT`

### 24.18 — Team Behind MV [HAIKYUU]
**Scene:** Full squad behind coach — silent — Logan mouths "go."
**Dialogue:** *(silent team support)*
**Prompt:** `full team behind coach corner silent support haikyuu, STYLE: HAIKYUU TOURNAMENT`

### 24.19 — Ethan Breath [KWONS]
**Scene:** Extreme close — Ethan's eyes — breath in — out — ring blur behind.
**Dialogue:** Ethan (internal): *Armor on.*
**Prompt:** `extreme close teen eyes breathing pre match focus, STYLE: KWONS TOURNAMENT`

### 24.20 — Ring Wide — Shi-jak Setup [ACTION]
**Scene:** Wide octagon — both fighters — ref center — crowd hush — chapter end.
**Dialogue:** Ref: "Ready…"
**Prompt:** `olympic taekwondo octagon wide both fighters referee ready chapter end, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

---

## Chapter 25 — Round 1: The Kick [CARD + TEACH + ACTION]

**Choreography:** [ACTION-CHOREOGRAPHY.md](../storyboards/ACTION-CHOREOGRAPHY.md) § Ch 25  
**Cards:** [CHARACTER-CARDS.md](../storyboards/CHARACTER-CARDS.md) § Ch 25

### 25.0 — Fighter VS Cards [CARD] ★
**Scene:** Dual stat card — **Ethan Hyun** (blue) vs **Derek Cole** (red). Empty boxes for post text.
**Overlay text:** Ethan · 15 · 5'3" · 95 lbs · Junior −45 kg · 3rd Dan · Kwon's | VS | Derek · 16 · 5'10" · 128 lbs · Open −54 kg · 2nd Dan · Apex TKD
**Prompt:** `dual fighter VS profile card taekwondo tournament blue red portraits empty stat boxes, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: CARD TOURNAMENT`

### 25.1 — Octagon Wide
**Scene:** Olympic-style mat — names on screen — score 0–0 — Round 1.
**Prompt:** `olympic taekwondo octagon wide establishing two fighters referee, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 25.2 — Guard Up
**Scene:** Ref bow. Ethan **counter stance** — low guard — Derek side-on bounce.
**Dialogue:** Ref: "Shi-jak!"
**Prompt:** `taekwondo match start two fighters kyorugi guard up referee shi-jak, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 25.3 — Probe Cut [ACTION]
**Scene:** Derek **front-leg cut kick** — Ethan blocks high — trunk sensor test.
**Dialogue:** MV (corner): "Cut. Don't chase."
**Prompt:** `front leg cut kick blocked guard olympic electronic hogu, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.4 — Ethan Step-In [ACTION]
**Scene:** Ethan **steps into range** — classic mistake vs spin setup.
**Dialogue:** MV (corner): "Spin—!"
**Prompt:** `fighter stepping into range too close taekwondo mistake, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.5 — Feint Cut & Chamber [TEACH + ACTION]
**Scene:** Derek **second cut feint** — hip drops — **spinning hook chamber** — slow-mo inset foot path.
**Prompt:** `cut kick feint into spinning hook kick chamber slow motion diagram inset, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH ACTION`

### 25.6 — Impact [TEACH + ACTION] ★
**Scene:** **Spinning hook** to head — PSS headgear **LED flash** — scoreboard **+5**.
**SFX:** *BEEP* · silence drop
**Prompt:** `spinning hook kick head impact electronic headgear red LED flash speed lines impact frame, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH ACTION`

### 25.7 — Ethan Frozen
**Scene:** Ethan stands — guard dropped — ref waves — Ethan doesn't move.
**Prompt:** `teen fighter frozen after head kick loss dissociation guard down, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 25.8 — Scoreboard
**Scene:** Derek leads **5–0** — round time remaining — psychological KO.
**Prompt:** `taekwondo scoreboard plus five turning head kick lead, STYLE: TOURNAMENT`

### 25.9 — MV Corner [HAIKYUU]
**Scene:** MV hand on barrier — doesn't yell — team behind silent.
**Dialogue:** *(silence)*
**Prompt:** `coach corner hand on barrier concerned team silent, STYLE: HAIKYUU TOURNAMENT`

### 25.10 — Nightmare Inset
**Scene:** Flash — **same kick** Arc 1 Ch 1 nightmare — visual rhyme.
**Prompt:** `flashback inset spinning hook kick same trauma visual rhyme desaturated, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER`

### 25.11 — Headgear Off
**Scene:** Ethan removes headgear — hands shake — eyes empty.
**Prompt:** `teen removing taekwondo headgear shaking hands trauma, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 25.12 — Match Over
**Scene:** Wide — Derek bows — Ethan late bow — crowd blur.
**Prompt:** `post match bow winner loser olympic taekwondo wide, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 25.13 — Ref Stoppage [ACTION]
**Scene:** Ref steps between — hand up — round clock still ticking — Ethan unresponsive.
**Dialogue:** Ref: "Kal-yeo!"
**Prompt:** `referee stopping round fighter unresponsive taekwondo, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.14 — Corner Reach [HAIKYUU]
**Scene:** MV hand on barrier — can't enter — jaw tight — Logan behind him straining.
**Dialogue:** MV: "Ethan. Look at me." · *(no response)*
**Prompt:** `coach hand on barrier cannot enter concerned corner, STYLE: HAIKYUU TOURNAMENT`

### 25.15 — Crowd Roar Mute [ACTION]
**Scene:** Crowd cheering — sound design **muted** — Ethan's POV underwater.
**SFX:** *(muffled distant)*
**Prompt:** `crowd cheering muted POV dissociation underwater sound, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.16 — Derek Respect [ACTION]
**Scene:** Derek doesn't celebrate — small nod at frozen Ethan — walks off professional.
**Dialogue:** Derek: "Good fight." · *(Ethan doesn't hear)*
**Prompt:** `winning fighter respectful nod opponent frozen professional, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.17 — Kieryn Fist [HAIKYUU]
**Scene:** Kieryn white-knuckles corner post — Ariana hand on her wrist — hold.
**Dialogue:** Kieryn: "Let MV—" · Ariana: "I know."
**Prompt:** `angry teen girl gripping corner post teammate restraining, STYLE: HAIKYUU`

### 25.18 — Walk Off Mat [ACTION]
**Scene:** Ethan steps wrong direction — ref guides — feet heavy — gear loose.
**Dialogue:** Ref: "This way."
**Prompt:** `disoriented teen fighter guided off mat heavy feet, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 25.19 — Scoreboard Final [TEACH]
**Scene:** Board locks — **Derek 5 — Ethan 0** — Round 1 — match called.
**Dialogue:** MV (internal): *Five points. One kick. That's all it took.*
**Prompt:** `taekwondo scoreboard five zero round one match over, STYLE: TOURNAMENT TEACH`

---

## Chapter 26 — Premier Opening [FLASH — REAL]

**Timeline:** **FLASH Premier** — triggered by Ch 25 loss. **PRESENT** resumes at **Ch 31.1**.

### 26.1 — Warm Opening [REAL]
**Scene:** Young Ethan white belt — Premier small dojang. Instructor smiles.
**Dialogue:** Instructor: "Welcome to the family."
**Prompt:** `flashback small dojang white belt boy bowing warm coach, STYLE: PREMIER lighter`

### 26.2 — First Bow [PREMIER]
**Scene:** Ethan copies instructor bow — too deep — almost falls — instructor laughs kindly.
**Dialogue:** Instructor: "Again. Less drama."
**Prompt:** `young white belt boy bowing too deep almost falling kind coach, STYLE: PREMIER`

### 26.3 — Small Dojang Wide [PREMIER]
**Scene:** Wide — mirrors — six students max — sunlight through front window.
**Dialogue:** Ethan (internal): *It felt like church.*
**Prompt:** `small taekwondo dojang wide warm sunlight few students, STYLE: PREMIER lighter`

### 26.4 — White Belt Line [PREMIER]
**Scene:** Kids lined by height — Ethan shortest — doesn't mind yet.
**Dialogue:** Instructor: "Discipline starts with showing up."
**Prompt:** `children lined by height white belts smallest boy smiling, STYLE: PREMIER`

### 26.5 — First Form [TEACH]
**Scene:** Instructor adjusts Ethan's chamber — hands gentle — correct angle.
**Dialogue:** Instructor: "Hip first. Then foot. You're listening — that's rare."
**Prompt:** `instructor correcting child taekwondo chamber gentle teaching, STYLE: PREMIER TEACH`

### 26.6 — Parent Watch [PREMIER soft]
**Scene:** Parents on bench — mother waves — Ethan waves back mid-form — corrected.
**Dialogue:** Instructor: "Eyes forward." · Ethan: "Sorry."
**Prompt:** `parents watching class child waving corrected soft, STYLE: PREMIER soft`

### 26.7 — Stripe Promotion [PREMIER]
**Scene:** New white belt stripe taped on — Ethan proud — genuine smile.
**Dialogue:** Instructor: "You earned that."
**Prompt:** `child receiving belt stripe proud smile warm dojang, STYLE: PREMIER lighter`

### 26.8 — Family Dinner Echo [PREMIER soft]
**Scene:** Ethan shows stripe at dinner table — fish on plate — small celebration.
**Dialogue:** Mother: "Next time we watch from the front row."
**Prompt:** `child showing belt stripe family dinner warm, STYLE: PREMIER soft`

### 26.9 — Instructor Praise [PREMIER]
**Scene:** Instructor ruffles hair — rare — Ethan beams.
**Dialogue:** Instructor: "Keep that attitude. Don't lose it."
**Prompt:** `coach ruffling child hair rare praise warm, STYLE: PREMIER lighter`

### 26.10 — Mirror Smile [PREMIER]
**Scene:** Young Ethan mirror — white belt — practices bow alone after class.
**Dialogue:** Ethan (internal): *I belong here.*
**Prompt:** `young boy practicing bow alone mirror white belt hopeful, STYLE: PREMIER lighter`

### 26.11 — First Sparring Touch [TEACH]
**Scene:** Light contact — instructor stops immediately — safe.
**Dialogue:** Instructor: "Control. Always control."
**Prompt:** `children light sparring instructor stopping safe control, STYLE: PREMIER TEACH`

### 26.12 — Yellow Belt Ceremony [PREMIER]
**Scene:** Small ceremony — five students — cookies after — community.
**Dialogue:** Instructor: "Welcome to the family — again."
**Prompt:** `small belt ceremony cookies community warm dojang, STYLE: PREMIER lighter`

### 26.13 — Ethan Helps New Kid [PREMIER]
**Scene:** Ethan shows new white belt how to tie belt — patient — instructor watches approving.
**Dialogue:** Instructor: "Teach him like someone taught you."
**Prompt:** `older child teaching new white belt tie approving coach, STYLE: PREMIER`

### 26.14 — Dojang Sign Close [PREMIER]
**Scene:** **Premier Taekwondo** sign — fresh paint — Ethan reads it walking in.
**Dialogue:** Ethan (internal): *Premier. Like the best.*
**Prompt:** `taekwondo school sign fresh paint child reading hopeful, STYLE: PREMIER`

### 26.15 — Rainy Day Class [PREMIER soft]
**Scene:** Half attendance — instructor still teaches full lesson — Ethan only student some days.
**Dialogue:** Instructor: "Rain doesn't cancel discipline."
**Prompt:** `rainy day single student lesson dedicated coach, STYLE: PREMIER soft`

### 26.16 — Photo Wall [PREMIER]
**Scene:** Dojang photo wall — tournament kids — Ethan points at empty space.
**Dialogue:** Instructor: "Your picture goes there someday."
**Prompt:** `dojang photo wall child pointing empty space dream, STYLE: PREMIER lighter`

### 26.17 — Green Belt Spar [ACTION]
**Scene:** Ethan scores clean body kick — instructor claps — real praise.
**Dialogue:** Instructor: "That was **clean**. Again."
**Prompt:** `child landing clean body kick coach clapping praise, STYLE: PREMIER ACTION`

### 26.18 — Last Warm Day [PREMIER]
**Scene:** Wide dojang — same angle as 26.3 — more students now — instructor smile thinner.
**Dialogue:** Ethan (internal): *I didn't notice when it changed.*
**Prompt:** `same dojang angle more students coach smile thinner unease beginning, STYLE: PREMIER`

### 26.19 — New Mats Delivered [PREMIER]
**Scene:** Delivery truck — new mats — parents help carry — excitement.
**Dialogue:** Instructor: "We're growing. Good problem."
**Prompt:** `new taekwondo mats delivery parents helping excited, STYLE: PREMIER`

### 26.20 — Chapter End — Smile Fades [PREMIER]
**Scene:** Instructor on phone in back office — door cracked — expression cold — Ethan glimpses — doesn't understand yet.
**Dialogue:** Ethan (internal): *Still family. Right?*
**Prompt:** `child glimpsing coach cold phone call door cracked unease chapter end, STYLE: PREMIER`

---

## Chapter 27 — School Grows [FLASH — REAL #8]

**Timeline:** **FLASH Premier** — continues dissociation cascade from Ch 25.

### 27.1 — Money Grows [REAL #8]
**Scene:** More students — instructor tells parents refs cheated.
**Dialogue:** Instructor: "They bought those belts. Your kids earned theirs."
**Prompt:** `coach lying to parents post tournament, STYLE: PREMIER`

### 27.2 — Crowded Mat [PREMIER]
**Scene:** Same dojang — triple students — Ethan bumped in form line — no correction.
**Dialogue:** Instructor: "Tighter. Everyone tighter." · *(doesn't demonstrate)*
**Prompt:** `overcrowded taekwondo class child bumped no individual correction, STYLE: PREMIER`

### 27.3 — Trophy Case [PREMIER]
**Scene:** New trophies behind glass — Premier logo — parents photograph.
**Dialogue:** Parent: "Number one school in the district!" · Instructor: "We earned it."
**Prompt:** `trophy case new trophies parents photographing proud, STYLE: PREMIER`

### 27.4 — Post-Tournament Lie [REAL #8]
**Scene:** Parents upset after loss — instructor arms crossed — blame external.
**Dialogue:** Instructor: "Judges were paid. Your kid was better. They knew it."
**Prompt:** `coach blaming judges to angry parents arms crossed, STYLE: PREMIER`

### 27.5 — Ethan Watches Lie [PREMIER]
**Scene:** Ethan hears from hallway — opponent was kind — shook his hand — cognitive dissonance.
**Dialogue:** Ethan (internal): *He wasn't cheating. He was just… better.*
**Prompt:** `child overhearing hallway coach lie cognitive dissonance, STYLE: PREMIER`

### 27.6 — Favored Student [PREMIER]
**Scene:** **Ha-eun** (composite) — perfect form — instructor adjusts only her — ignores Ethan.
**Dialogue:** Instructor: "Like this. Perfect." · *(to Ha-eun only)*
**Prompt:** `coach adjusting favored girl form ignoring other students, STYLE: PREMIER`

### 27.7 — Parent Recruitment [PREMIER]
**Scene:** Instructor pitches black belt package to parents — price visible on flyer — Ethan in background.
**Dialogue:** Instructor: "Black belt by fourteen builds college apps."
**Prompt:** `coach selling black belt package parents flyer price, STYLE: PREMIER`

### 27.8 — Less Correction [TEACH]
**Scene:** Split — old memory: instructor fixes stance — new: Ethan wrong — instructor walks past.
**Dialogue:** Ethan (internal): *When did he stop seeing me?*
**Prompt:** `split panel coach correcting then ignoring same student contrast, STYLE: PREMIER TEACH`

### 27.9 — Ttong Kim Nameplate [PREMIER]
**Scene:** Office door — **Master Ttong Kim** plaque — new — formal photo stern.
**Dialogue:** Parent: "He seems… serious now." · Other parent: "Serious wins."
**Prompt:** `dojo office master nameplate stern formal photo, STYLE: PREMIER`

### 27.10 — Belt Test Fees [PREMIER]
**Scene:** Line at desk — parents paying — Ethan holds number ticket — conveyor belt energy.
**Dialogue:** Desk: "Testing fee. Board fee. Video fee."
**Prompt:** `parents paying belt test fees line child holding number, STYLE: PREMIER`

### 27.11 — Demo Team Selection [PREMIER]
**Scene:** Ha-eun picked — Ethan not — instructor doesn't explain.
**Dialogue:** Instructor: "Demo team — front row at regionals." · *(Ethan not called)*
**Prompt:** `coach selecting demo team favored students others excluded, STYLE: PREMIER`

### 27.12 — Ethan Trains Alone [PREMIER]
**Scene:** After class — Ethan practices forms — empty mat — instructor on phone elsewhere.
**Dialogue:** Ethan (internal): *Harder. Maybe he'll look.*
**Prompt:** `child practicing forms alone empty mat after class, STYLE: PREMIER`

### 27.13 — Ref Blame Callback [REAL #8]
**Scene:** TV in lobby — real tournament — instructor mutes — different narrative.
**Dialogue:** Instructor: "See that? Rigged." · Ethan: "…That's WT electronic scoring."
**Prompt:** `coach blaming TV tournament lobby child quiet objection, STYLE: PREMIER`

### 27.14 — Cold Correction [PREMIER]
**Scene:** Ethan finally corrected — harsh — public — different from Ch 26.
**Dialogue:** Instructor: "Sloppy. You're making my school look bad."
**Prompt:** `coach harsh public correction embarrassed teen, STYLE: PREMIER`

### 27.15 — Parents Buy In [PREMIER]
**Scene:** Parent group chat inset — "Master Kim says refs cheat" — echo chamber.
**Dialogue:** *(text overlay)* "We're not leaving. He's the best."
**Prompt:** `phone parent group chat echo chamber coach narrative, STYLE: PREMIER`

### 27.16 — Red Belt Test Notice [PREMIER]
**Scene:** Flyer — **Black Belt Testing — 6 months** — Ethan stares — hand tight.
**Dialogue:** Ethan (internal): *Six months. I can do six months.*
**Prompt:** `black belt testing flyer six months teen staring determined, STYLE: PREMIER`

### 27.17 — Sparring Rougher [ACTION]
**Scene:** Senior students hit hard — instructor doesn't stop — "toughen up."
**Dialogue:** Instructor: "Pain teaches. Don't cry."
**Prompt:** `rough sparring senior students coach not stopping toughen up, STYLE: PREMIER ACTION`

### 27.18 — Mirror Change [PREMIER]
**Scene:** Teen Ethan mirror — red belt — same mirror as 26.10 — no smile.
**Dialogue:** Ethan (internal): *Same mirror. Different kid.*
**Prompt:** `teen red belt mirror no smile same angle contrast, STYLE: PREMIER`

### 27.19 — Testing Day Announced [PREMIER]
**Scene:** Instructor claps — testing list — Ethan's name — no warmth in congratulations.
**Dialogue:** Instructor: "Don't embarrass me."
**Prompt:** `coach announcing testing list cold don't embarrass me, STYLE: PREMIER`

### 27.20 — Gymnasium Chairs [PREMIER]
**Scene:** Wide — chairs in rows — parents with cameras — testing day morning — chapter end into Ch 28.
**Dialogue:** Instructor: "Perform."
**Prompt:** `gymnasium chairs rows parents cameras belt testing day wide, STYLE: PREMIER`

---

## Chapter 28 — Board Break [FLASH — REAL #10, #24] ★

**Timeline:** **FLASH Premier** — belt test board station.  
**Board break script:** [flash-premier-board-break.md](../scripts/flash-premier-board-break.md)

### 28.1 — Board Station [REAL #24] ★
**Scene:** Testing day — **break board station** — Ethan **smallest, youngest** in line — adult-sized board.
**Dialogue:** Instructor: "Punch through. Next."
**Prompt:** `flashback taekwondo belt test board break station smallest teen in group, STYLE: PREMIER`

### 28.2 — First Failures [FLASH — belt test, dobok only]
**Scene:** Ethan punches **below center line** — board holds — knuckles pink — **Ttong Kim watches, says nothing** — other kids finished.
**Dialogue:** Parent (O.S. whisper): "Why don't they **move the board closer**?"
**SFX:** *thud* *thud* *thud*
**Prompt:** `teen punching wooden board wrong angle failing knuckles reddening silent coach white dobok belt test NO hogu, STYLE: PREMIER`

### 28.3 — Clock Moves [REAL #24] [FLASH — belt test]
**Scene:** Wall clock — **~almost an hour** — parents standing — **instructor never corrects stance or board height** — Ethan won't stop.
**Dialogue:** Parent (O.S.): "Please— just let him stop—" · Parent (O.S. whisper): "He's not even hitting the **center**—"
**Prompt:** `wall clock hour passing teen still punching board wrong position coach silent white dobok belt test, STYLE: PREMIER`

### 28.4 — Board Breaks
**Scene:** Ethan **shifts foot forward, adjusts hip** — board **splits** — not heroic — Ethan stumbles — **hand black and blue**, swollen.
**Dialogue:** Ethan (internal): *It broke when I moved. Not when they helped.*
**Prompt:** `teen adjusts stance finally breaks board exhausted not triumphant bruised fist, STYLE: PREMIER`

### 28.5 — One of Five [REAL #24]
**Scene:** Grader writes **1/5** — instructor face in background — **disgust, disappointment** — not anger — worse.
**Dialogue:** Instructor: "Late and ugly isn't the same as good. **One out of five.**"
**Prompt:** `instructor disappointed disgusted face grading sheet one out of five, STYLE: PREMIER`

### 28.6 — Crowd Crying & Whispers [REAL #24] [FLASH — belt test]
**Scene:** Bleachers — parents **crying** — **after break, louder whispers**: move board closer · wrong position whole hour · **coach should've said one word** — Ethan numb.
**Dialogue:** Parent (O.S.): "Why didn't the instructor **adjust** anything?" · Father (O.S., hoarse): "He figured it out **himself**."
**Prompt:** `parents crying whispering belt test board positioning failure coach silence white dobok, STYLE: PREMIER soft`

### 28.7 — Results Board [REAL #9, #24]
**Scene:** Final scores — Ethan **highest overall passing** — then instructor adds **bonus** to favored girl — Ethan **2nd**.
**Dialogue:** Instructor (to girl): "Extra credit. Attitude." · Ethan (internal): *I would have passed with zero.*
**Prompt:** `test results board teen demoted second place favoritism bonus points, STYLE: PREMIER`

### 28.8 — First to Black Belt [REAL #10]
**Scene:** Black belt ceremony — Ethan empty eyes. Parents clapping.
**Dialogue:** Ethan (internal): *Nothing left but fear.*
**Prompt:** `black belt ceremony teen hollow expression parents applauding bruised hand hidden, STYLE: PREMIER`

### 28.9 — Board Holder Wrong Height [TEACH]
**Scene:** Diagram inset — board center line above Ethan's fist height — volunteer holds steady.
**Dialogue:** Parent (O.S. whisper): "Shouldn't they lower it?"
**Prompt:** `board break height diagram wrong position teen too short educational, STYLE: PREMIER TEACH`

### 28.10 — Ttong Kim Silent [PREMIER]
**Scene:** Instructor watches — arms behind back — does not move — clock visible.
**Dialogue:** Instructor: *(to another parent)* "Good power. Needs commitment."
**Prompt:** `coach arms behind back watching child fail board silent, STYLE: PREMIER`

### 28.11 — Knuckles Pink [PREMIER]
**Scene:** Extreme close — Ethan's knuckles — pink → red — board grain unbroken.
**SFX:** *thud*
**Prompt:** `extreme close bruising knuckles wooden board unbroken, STYLE: PREMIER`

### 28.12 — Other Kids Done [PREMIER]
**Scene:** Background — other testers at forms/sparring stations — finished — Ethan still at board.
**Dialogue:** Ethan (V.O.): *Other kids went to forms. I was still here.*
**Prompt:** `background testers finished stations teen still at board alone, STYLE: PREMIER`

### 28.13 — Mother Stands [PREMIER soft]
**Scene:** Mother rises from bleacher — instructor hand wave — sit down.
**Dialogue:** Instructor: "He'll break it or he won't. Sit down."
**Prompt:** `mother standing worried coach hand wave sit down, STYLE: PREMIER soft`

### 28.14 — Quitting Speech [PREMIER]
**Scene:** Instructor to mother — not looking at Ethan — cruelty as lesson.
**Dialogue:** Instructor: "Then let him learn what quitting looks like."
**Prompt:** `coach dismissing parent concern quitting lesson cold, STYLE: PREMIER`

### 28.15 — Foot Shift [ACTION]
**Scene:** Slow-mo — Ethan **one foot forward** — hip rotates — fist path changes — alone.
**Dialogue:** Ethan (internal): *There.*
**Prompt:** `slow motion teen adjusting foot hip board break alone discovery, STYLE: PREMIER ACTION`

### 28.16 — Board Splits [PREMIER]
**Scene:** Board cracks — not heroic — Ethan stumbles forward — exhausted.
**SFX:** *crack*
**Prompt:** `board finally breaking teen stumbling exhausted not triumphant, STYLE: PREMIER`

### 28.17 — Swollen Hand Hide [PREMIER]
**Scene:** Ethan tucks hand behind back — walking to line — grader waiting.
**Dialogue:** Grader: "Next."
**Prompt:** `teen hiding swollen hand behind back walking to grader, STYLE: PREMIER`

### 28.18 — Ha-eun Bonus [REAL #9]
**Scene:** Grader writes — Ha-eun **5/5** — instructor nod — Ethan watches.
**Dialogue:** Instructor: "Extra credit. Attitude."
**Prompt:** `grader writing perfect score favored student coach nod favoritism, STYLE: PREMIER`

### 28.19 — Zero Still Passes [TEACH]
**Scene:** MV-style inset (memory) — rules sheet — **board 0/5 = still pass** — Ethan didn't know.
**Dialogue:** Ethan (V.O.): *I would have passed with zero.*
**Prompt:** `belt test rules sheet board zero still passes educational inset, STYLE: PREMIER TEACH`

### 28.20 — Hollow Black Belt [PREMIER]
**Scene:** Callback 28.8 wide — belt tied — applause — Ethan's eyes nowhere — chapter end.
**Dialogue:** Father (O.S., hoarse): "He figured it out **himself**."
**Prompt:** `black belt tied hollow eyes applause contrast chapter end, STYLE: PREMIER`

---

## Chapter 29 — Go Fight [FLASH — REAL #13]

**Timeline:** **FLASH Premier** — black belt exhibition match.

### 29.1 — Go Fight [REAL #13]
**Scene:** No warmup. Instructor points. **"Go fight."**
**Prompt:** `coach pointing at ring dismissive teen cold no warmup, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER`

### 29.2 — Spinning Hook Anatomy [TEACH]
**Scene:** Slow-mo flashback — foot path arc, pivot foot labeled, head target zone.
**Dialogue:** *(MV voice memory)* "Same kick. You just didn't know its name yet."
**Prompt:** `spinning hook kick anatomical diagram foot path educational flashback, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER TEACH`

### 29.3 — Exhibition Ring [PREMIER]
**Scene:** Wide — Premier gym — exhibition mat — crowd smaller than tournament — heavier opponent waiting.
**Dialogue:** Opponent: *(stretching, bored)*
**Prompt:** `exhibition taekwondo ring teen vs seasoned athlete premier gym, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER`

### 29.4 — No Corner [PREMIER]
**Scene:** Ethan looks to corner — empty chair — instructor talking to parents.
**Dialogue:** Ethan (internal): *No one. Again.*
**Prompt:** `teen looking at empty corner chair coach ignoring, STYLE: PREMIER`

### 29.5 — Opponent Size [TEACH]
**Scene:** Height comparison inset — opponent **cadet/open** seasoned — Ethan new black belt.
**Dialogue:** Instructor (O.S.): "Points. Go."
**Prompt:** `height size comparison teen vs seasoned athlete educational, STYLE: PREMIER TEACH`

### 29.6 — Cold Stance [ACTION]
**Scene:** Ethan guard up — feet wrong — no bounce — opponent circles easy.
**Dialogue:** Ethan (internal): *Go fight. That's all I got.*
**Prompt:** `teen stiff guard no warmup cold stance opponent circling, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.7 — Opponent Probe [ACTION]
**Scene:** Opponent light cut kicks — testing — Ethan flinches late.
**Prompt:** `seasoned fighter probe cut kicks teen flinching late, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.8 — No Advice Memory [PREMIER]
**Scene:** Inset — other schools' corners — coaches whisper — Ethan alone.
**Dialogue:** Ethan (internal): *They get words. I get pointed.*
**Prompt:** `split inset other coaches cornering vs empty corner contrast, STYLE: PREMIER`

### 29.9 — Feint Setup [ACTION]
**Scene:** Opponent feint — hip drop — same tell as Derek — Ethan doesn't recognize yet.
**Prompt:** `opponent feint hip drop spin setup premier exhibition, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.10 — Step In Mistake [ACTION]
**Scene:** Ethan closes distance — same mistake as Ch 25 — visual rhyme building.
**Dialogue:** Instructor (O.S.): *(nothing)*
**Prompt:** `teen stepping into range spin trap mistake rhyme, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.11 — Chamber Slow-Mo [TEACH + ACTION]
**Scene:** Spinning hook chamber — labeled — pivot foot — head zone — identical to 25.5.
**Prompt:** `spinning hook chamber slow motion diagram premier exhibition, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER TEACH ACTION`

### 29.12 — Instructor Watching [PREMIER]
**Scene:** Ttong Kim arms crossed — no concern — watching like inventory.
**Dialogue:** Instructor: *(flat)* "…"
**Prompt:** `coach arms crossed watching student fight no concern, STYLE: PREMIER`

### 29.13 — Parents Cringe [PREMIER soft]
**Scene:** Parents in crowd — mother half-rises — father hand on her arm.
**Dialogue:** Mother: "—" · Father: "Sit."
**Prompt:** `parents cringing watching child fight mother half rising, STYLE: PREMIER soft`

### 29.14 — Guard Drop Foreshadow [ACTION]
**Scene:** Ethan guard high — fatigue — hands drift — opponent reads it.
**Prompt:** `teen guard drifting fatigue opponent reading opening, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.15 — MV Echo [TEACH]
**Scene:** Present-day MV voice over flash — *Cover high* — too late in memory.
**Dialogue:** MV (memory overlay): "Cover high."
**Prompt:** `coach voice memory overlay flashback cover high too late, STYLE: PREMIER TEACH`

### 29.16 — Three-Kick Rhyme [PREMIER]
**Scene:** Triptych inset — Arc 1 nightmare · Ch 25 Derek · Ch 29 exhibition — same composition.
**Prompt:** `triptych three spinning hook kicks visual rhyme desaturated, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER`

### 29.17 — Pre-Impact Breath [ACTION]
**Scene:** Ethan exhales — guard still dropping — heel approaching — silence before.
**SFX:** *(silence)*
**Prompt:** `pre impact silence guard dropping heel approaching, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 29.18 — Crowd Gasp [PREMIER]
**Scene:** Crowd reacts — instructor unchanged — opponent doesn't celebrate — clinical.
**Prompt:** `crowd gasping head kick coach unchanged clinical, STYLE: PREMIER`

### 29.19 — Points on Board [TEACH]
**Scene:** Manual score — opponent +3 head — Ethan 0 — exhibition continues.
**Dialogue:** Grader: "Chung. Three."
**Prompt:** `manual scoreboard head kick three points exhibition, STYLE: PREMIER TEACH`

### 29.20 — Match Continues Broken [ACTION]
**Scene:** Ethan still standing — guard gone — opponent backs off confused — ref waves continue — chapter end into Ch 30.
**Dialogue:** Ref: "Gye-sok!"
**Prompt:** `teen standing guard down opponent confused referee continue, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

---

## Chapter 30 — Twenty Minute Scold [FLASH — REAL #11–12]

**Timeline:** **FLASH Premier** — aftermath of head kick.

### 30.1 — The Kick Lands [REAL #11]
**Scene:** Identical head impact — younger Ethan drops guard.
**Prompt:** `flashback head kick impact teen dropping guard, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER`

### 30.2 — Twenty Minute Scold [REAL #12]
**Scene:** Instructor yelling — clock on wall moves. Ethan tears.
**Dialogue:** Instructor: "You took a head shot like you wanted to lose!"
**Prompt:** `coach yelling at crying teen long time passage clock wall, STYLE: PREMIER`

### 30.3 — Impact Echo [ACTION]
**Scene:** Head snap — sensorless Premier gear — mouthguard visible — stars in vision.
**Prompt:** `head kick impact stars vision mouthguard premier gear, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: PREMIER ACTION`

### 30.4 — Guard on Floor [PREMIER]
**Scene:** Ethan's hands at sides — headgear askew — opponent bowing already.
**Dialogue:** Opponent: "You okay?" · Ethan: *(doesn't answer)*
**Prompt:** `teen hands at sides headgear askew opponent concerned bow, STYLE: PREMIER`

### 30.5 — Corner Drag [PREMIER]
**Scene:** Instructor grabs Ethan's arm — not gentle — pulls to corner — no water.
**Dialogue:** Instructor: "Stand up. You're embarrassing me."
**Prompt:** `coach grabbing teen arm pulling corner harsh no water, STYLE: PREMIER`

### 30.6 — Clock Five Minutes [PREMIER]
**Scene:** Wall clock — **5:00** — instructor still talking — Ethan staring at floor.
**Dialogue:** Instructor: "Black belt doesn't mean soft—"
**Prompt:** `wall clock five minutes coach still yelling teen staring floor, STYLE: PREMIER`

### 30.7 — Parents Look Away [PREMIER soft]
**Scene:** Parents in crowd — some look away — mother doesn't — tears held.
**Dialogue:** Mother: *(whisper)* "Enough."
**Prompt:** `parents looking away mother holding tears watching, STYLE: PREMIER soft`

### 30.8 — Ten Minutes [PREMIER]
**Scene:** Clock **10:00** — instructor pacing — Ethan tears hit chin — no wipe.
**Dialogue:** Instructor: "You want pity? Go to another school."
**Prompt:** `clock ten minutes coach pacing crying teen tears chin, STYLE: PREMIER`

### 30.9 — Other Students Watch [PREMIER]
**Scene:** Kids peek from hallway — Ha-eun looks — looks away — lesson internalized wrong.
**Dialogue:** Ha-eun (internal): *Don't be weak.*
**Prompt:** `students peeking hallway favored girl looking away lesson, STYLE: PREMIER`

### 30.10 — Fifteen Minutes [PREMIER]
**Scene:** Clock **15:00** — Ethan's fists clenched — not anger — shutdown.
**Dialogue:** Instructor: "Head up when I'm talking!"
**Prompt:** `clock fifteen minutes teen shutdown fists clenched, STYLE: PREMIER`

### 30.11 — Word Loop [PREMIER]
**Scene:** Instructor mouth repeated — *wanted to lose* — echo distortion — Solo Leveling dread.
**Dialogue:** Instructor: "You **wanted** to lose— you **wanted**—"
**Prompt:** `coach mouth echo distortion repeated words dread atmosphere, STYLE: PREMIER`

### 30.12 — Twenty Minutes [REAL #12]
**Scene:** Clock **20:00** — instructor finally stops — Ethan shaking — not from kick — from words.
**Dialogue:** Instructor: "…Get water. Don't cry in my gym again."
**Prompt:** `clock twenty minutes coach finished teen shaking trauma words, STYLE: PREMIER`

### 30.13 — Water Fountain [PREMIER soft]
**Scene:** Ethan alone at fountain — water runs — doesn't drink — reflection broken.
**Dialogue:** Ethan (internal): *Same kick. Same freeze.*
**Prompt:** `teen alone water fountain not drinking broken reflection, STYLE: PREMIER soft`

### 30.14 — Headgear Off Flash [PREMIER]
**Scene:** Rhyme — Ch 25.11 present + Ch 30 flash — same hands shake removing headgear.
**Prompt:** `visual rhyme removing headgear shaking hands present flash overlay, STYLE: PREMIER TOURNAMENT`

### 30.15 — Drive Home Silent [PREMIER]
**Scene:** Car backseat — Ethan stares out — parents don't speak — bruise forming on cheek.
**Dialogue:** *(silence)*
**Prompt:** `car backseat teen staring window bruised cheek parents silent, STYLE: PREMIER`

### 30.16 — Nightmare Seed [PREMIER]
**Scene:** Bedroom ceiling — same kick replays — Arc 1 nightmare born — desaturated.
**Dialogue:** Ethan (internal): *It never left.*
**Prompt:** `bedroom ceiling nightmare head kick replay desaturated, STYLE: PREMIER`

### 30.17 — School Next Day [PREMIER]
**Scene:** Ethan in class — hoodie — knuckles wrapped — teacher doesn't ask.
**Dialogue:** Ethan (internal): *Perform. Don't cry. Don't quit.*
**Prompt:** `teen class hoodie wrapped knuckles quiet trauma, STYLE: PREMIER`

### 30.18 — Premier Logo Burn [PREMIER]
**Scene:** Ethan stares at Premier bag — considers — keeps going — fear not choice.
**Dialogue:** Ethan (internal): *Where else would I go?*
**Prompt:** `teen staring premier gear bag fear not choice, STYLE: PREMIER`

### 30.19 — Cascade End [PREMIER]
**Scene:** All flash images collapse into single point — white — Ethan's pupil — present bleachers approaching.
**Prompt:** `flashback cascade collapsing single point pupil transition, STYLE: PREMIER TOURNAMENT`

### 30.20 — Snap to Present [TOURNAMENT]
**Scene:** Arena color floods back — Ethan on bleachers — gear half off — breath in — chapter end.
**Dialogue:** Ethan (internal): *…I'm here.*
**Prompt:** `color flooding back arena bleachers teen dissociation ending chapter, STYLE: TOURNAMENT`

---

## Chapter 31 — Back to Present [HAIKYUU]

### 31.1 — Alone Bleachers
**Scene:** Ethan seated — gear half off. Arena blur around him.
**Prompt:** `teen alone tournament bleachers trauma aftermath, STYLE: TOURNAMENT`

### 31.2 — Logan Sits [HAIKYUU]
**Scene:** Logan sits beside — no words. Shoulders touch.
**Dialogue:** *(silence)*
**Prompt:** `friend sitting silently beside devastated teammate haikyuu support, STYLE: HAIKYUU`

### 31.3 — TJ Line [HAIKYUU]
**Scene:** TJ crouches — eye level.
**Dialogue:** TJ: "That kick didn't win him the war. Next ring is ours."
**Prompt:** `captain crouching eye level pep talk injured teammate spirit, STYLE: HAIKYUU`

### 31.4 — MV Distant [KWONS]
**Scene:** MV at far corner — assistant beside — quiet exchange.
**Dialogue:** MV: "Same kick. He just didn't know its name yet. Now he does."
**Prompt:** `coach distant corner quiet exchange assistant tournament, STYLE: KWONS`

### 31.5 — Arena Blur Returns [TOURNAMENT]
**Scene:** Ethan's POV — color bleeding in — crowd noise gradual return.
**Dialogue:** Ethan (internal): *Ring four. Still going.*
**Prompt:** `POV color returning crowd noise gradual bleachers, STYLE: TOURNAMENT`

### 31.6 — Gear Half Off [TOURNAMENT]
**Scene:** Headgear in lap — chest hogu loosened — forearm guards still on — detail.
**Dialogue:** *(none)*
**Prompt:** `teen partial gear removed lap headgear hogu loosened bleachers, STYLE: TOURNAMENT`

### 31.7 — Kieryn Approaches [HAIKYUU]
**Scene:** Kieryn walks over — doesn't sit yet — blocks sun from Ethan's eyes.
**Dialogue:** Kieryn: "You breathing?"
**Prompt:** `teen girl blocking sun for teammate bleachers concern, STYLE: HAIKYUU`

### 31.8 — Ethan Nods [KWONS]
**Scene:** Small nod — not okay — but present.
**Dialogue:** Ethan: "…Yeah."
**Prompt:** `quiet teen small nod not okay but present, STYLE: KWONS`

### 31.9 — Kian Water [HAIKYUU]
**Scene:** Kian tosses water bottle — Ethan catches — doesn't open yet.
**Dialogue:** Kian: "Drink when you want. Not when we want."
**Prompt:** `teammate tossing water bottle bleachers support, STYLE: HAIKYUU`

### 31.10 — Ariana Bandage Check [KWONS]
**Scene:** Ariana glances at Ethan's hands — no injury — sits other side — triangle support.
**Dialogue:** Ariana: "Hands okay?"
**Prompt:** `girl checking teammate hands sitting support triangle, STYLE: KWONS`

### 31.11 — Logan Still Silent [HAIKYUU]
**Scene:** Callback 31.2 — Logan hasn't moved — shoulders still touching — time passed.
**Dialogue:** *(silence — minutes)*
**Prompt:** `friend still sitting silent shoulder touch time passed support, STYLE: HAIKYUU`

### 31.12 — Ring Call TJ [TOURNAMENT]
**Scene:** PA — **Lim, TJ — Ring 4** — team stirs — Ethan doesn't.
**Dialogue:** PA: "Blue corner — Lim, TJ."
**Prompt:** `tournament PA call teammate name team stirring, STYLE: TOURNAMENT`

### 31.13 — TJ Stands [HAIKYUU]
**Scene:** TJ rises — looks at Ethan — doesn't ask him to come — respects space.
**Dialogue:** TJ: "Watch if you can. No pressure."
**Prompt:** `captain standing looking at hurt teammate no pressure, STYLE: HAIKYUU`

### 31.14 — Team Splits [HAIKYUU]
**Scene:** Half to corner — half stay with Ethan — organic — no speech.
**Prompt:** `team splitting corner vs bleachers organic support, STYLE: HAIKYUU`

### 31.15 — Ethan Watches Walk [KWONS]
**Scene:** Ethan watches TJ walk to ring — back straight — anchor.
**Dialogue:** Ethan (internal): *He's not alone.*
**Prompt:** `teen watching captain walk to ring bleachers anchor, STYLE: KWONS TOURNAMENT`

### 31.16 — Premier Stare Bleachers [TOURNAMENT]
**Scene:** Premier students in stands — stare at Ethan — he looks away — not flinch — tired.
**Dialogue:** Ethan (internal): *Not today.*
**Prompt:** `premier students staring bleachers teen tired not flinching, STYLE: TOURNAMENT`

### 31.17 — MV Glance Back [KWONS]
**Scene:** MV walking to corner — one glance at bleachers — Ethan — nod — continues.
**Dialogue:** MV: *(micro nod)*
**Prompt:** `coach glancing back bleachers micro nod trust, STYLE: KWONS`

### 31.18 — Bottle Opens [KWONS]
**Scene:** Ethan finally opens water — drinks — hand steadier — small win.
**Dialogue:** Ethan (internal): *Still here.*
**Prompt:** `teen drinking water hand steadier small recovery, STYLE: KWONS`

### 31.19 — Ring Four Wide [TOURNAMENT]
**Scene:** Wide — TJ entering ring — Ethan small in bleachers — same ring Ethan lost on.
**Dialogue:** Logan: "Send him."
**Prompt:** `wide same ring new fighter entering bleachers hurt teammate watching, STYLE: TOURNAMENT HAIKYUU`

### 31.20 — Chapter End — Next Ring [HAIKYUU]
**Scene:** TJ bows in ring — team roars — Ethan whisper barely audible.
**Dialogue:** Ethan: "…Go." · *(chapter end)*
**Prompt:** `team roaring corner teen whispering go chapter end, STYLE: HAIKYUU TOURNAMENT`

---

## Chapter 32 — TJ's Match [CARD + TEACH + HAIKYUU]

### 32.0 — TJ Lim Card [CARD]
**Scene:** Single pre-match card — TJ · 15 · 5'11" · 150 lbs · Junior −68 kg · 3rd Dan · Kwon's
**Prompt:** `single fighter profile card tall muscular teen headband empty stats, STYLE: CARD TOURNAMENT`

### 32.1 — Corner Wall [HAIKYUU]
**Scene:** Full team at corner — Kieryn holds scorecard, Logan screams legally.
**Dialogue:** Logan: "SEND HIM TO THE MOON!"
**Prompt:** `team cheering corner wildly haikyuu bench energy, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU TOURNAMENT`

### 32.2 — Emperor's Kick [TEACH]
**Scene:** TJ power body kick — scoreboard **+2**. Crowd erupts.
**Dialogue:** MV: "Two! Drive through!"
**Prompt:** `powerful taekwondo body kick emperor kick scoreboard plus two, STYLE: TOURNAMENT TEACH`

### 32.3 — TJ Wins
**Scene:** Ref raises TJ arm. Team pile-on — Ethan stays edge, pulled in by Logan.
**Prompt:** `team celebrating fighter win group hug pulling quiet member in, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU`

### 32.4 — Opening Exchange [ACTION]
**Scene:** TJ blocks cut — answers with push kick — distance control.
**Dialogue:** MV: "Good range. Don't trade."
**Prompt:** `tall muscular fighter push kick distance control olympic, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.5 — Emperor Setup [TEACH]
**Scene:** TJ feints low — opponent drops guard — chamber diagram inset.
**Dialogue:** MV: "There. Body first."
**Prompt:** `body kick setup feint low guard drop diagram inset, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH`

### 32.6 — Drive Through [ACTION]
**Scene:** TJ hip drives — trunk sensor flash — follow-through held.
**SFX:** *BEEP*
**Prompt:** `power body kick drive through electronic hogu flash, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.7 — Corner Erupts [HAIKYUU]
**Scene:** Logan jumps — legal corner zone — MV fist pump — Kieryn tracks score.
**Dialogue:** Logan: "THAT'S THE EMPEROR!"
**Prompt:** `corner erupting legal celebration fist pump score tracking, STYLE: HAIKYUU TOURNAMENT`

### 32.8 — Scoreboard +2 [TEACH]
**Scene:** Board **TJ 2 — Opponent 0** — crowd erupts.
**Dialogue:** Logan: "TWO! THAT'S TWO!"
**Prompt:** `scoreboard plus two crowd erupting corner, STYLE: TOURNAMENT TEACH`

### 32.9 — Opponent Gamble [ACTION]
**Scene:** Opponent spins — TJ covers — blocks — no panic.
**Dialogue:** TJ (internal): *Not today.*
**Prompt:** `fighter blocking spinning kick calm cover, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.10 — Activity Round [ACTION]
**Scene:** TJ presses — cut exchanges — Olympic volume — clock ticks.
**Prompt:** `cut kick exchange activity scoring pressure olympic, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.11 — Corner Signals [TEACH]
**Scene:** MV hand signals — fist — palm down — TJ reads.
**Dialogue:** MV: *(signal)* "Control."
**Prompt:** `coach corner hand signals fighter reading, STYLE: TOURNAMENT TEACH`

### 32.12 — Round End [ACTION]
**Scene:** Ref stops — TJ ahead on board — controlled breathing.
**Dialogue:** Ref: "Kal-yeo!"
**Prompt:** `round end fighter ahead controlled breathing, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.13 — Ethan Stands [HAIKYUU]
**Scene:** Bleachers — Ethan stands to see — Logan pulls him forward — toward corner.
**Dialogue:** Logan: "Come on. He pulled you in after."
**Prompt:** `hurt teen standing bleachers pulled toward corner support, STYLE: HAIKYUU`

### 32.14 — Round Two Open [ACTION]
**Scene:** Shi-jak — TJ aggressive now — Blue Lock intensity — legal.
**Dialogue:** Ref: "Shi-jak!"
**Prompt:** `round two aggressive legal offense blue lock intensity, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.15 — Second Body +2 [ACTION]
**Scene:** Back leg roundhouse — trunk flash — opponent stumbles.
**SFX:** *BEEP*
**Prompt:** `back leg roundhouse body kick opponent stumbling, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.16 — Match Point [TEACH]
**Scene:** Scoreboard **4–1** — one round format story — match point energy.
**Dialogue:** Kieryn: "Close it."
**Prompt:** `scoreboard match point corner intensity, STYLE: TOURNAMENT TEACH`

### 32.17 — TJ Closes [ACTION]
**Scene:** TJ closes distance — punch +1 clinch break — professional finish.
**Prompt:** `clinch break punch plus one professional finish, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 32.18 — Arm Raised [HAIKYUU]
**Scene:** Ref raises TJ — team explodes — MV fist pump rare.
**Dialogue:** MV: "That's anchor."
**Prompt:** `referee raising winner arm team exploding rare coach fist pump, STYLE: HAIKYUU TOURNAMENT`

### 32.19 — Logan Pulls Ethan In [HAIKYUU]
**Scene:** Pile-on — Ethan at edge — Logan yanks sleeve — Kieryn pushes from behind — TJ hand on Ethan's shoulder — one squeeze.
**Dialogue:** Kieryn: "You're still on this team." · TJ: "Your turn next tournament." · Ethan: "…Yeah."
**Prompt:** `team hug pulling reluctant member in captain shoulder squeeze chapter end, STYLE: HAIKYUU KWONS`

---

## Chapter 33 — Everyone Fights [CARD + TEACH + ACTION]

**Cards:** [CHARACTER-CARDS.md](../storyboards/CHARACTER-CARDS.md) · **Choreography:** [ACTION-CHOREOGRAPHY.md](../storyboards/ACTION-CHOREOGRAPHY.md) § Ch 33

### 33.0 — Kieryn VS Card [CARD]
**Scene:** Dual card — **Kieryn Lim** vs **Maya Ortiz** before her match.
**Overlay:** Kieryn · 13 · 5'8" · 108 lbs · Cadet −49 kg · 2nd Dan | VS | Maya · 14 · 5'9" · 112 lbs · Cadet −49 kg · 2nd Dan · Valley Martial Arts
**Prompt:** `dual fighter VS profile card girl fighters cadet division empty stat boxes, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: CARD TOURNAMENT`

### 33.1 — Logan Neverending Rage [HAIKYUU]
**Scene:** Logan down on points — keeps charging. Crowd loves him.
**Dialogue:** Logan (internal): *One more step. One more.*
**Prompt:** `short fighter charging never give up crowd cheering underdog, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU TOURNAMENT`

### 33.1b — Logan Card [CARD] *(optional)*
**Scene:** Single card — Logan · 13 · 4'10" · 84 lbs · Cadet −41 kg · 2nd Dan
**Prompt:** `single fighter profile card short teen underdog taekwondo empty stats, STYLE: CARD`

### 33.2 — Kieryn Cut Probe [ACTION]
**Scene:** Round open — Kieryn **front-leg cut kick** — Maya blocks — **+2** if clean.
**Prompt:** `girl fighter front leg cut kick electronic hogu tap olympic, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.3 — Second Cut — Trap [ACTION]
**Scene:** Kieryn **second cut** — Maya **steps in** to answer — trap sprung.
**Dialogue:** Kieryn (internal): *There.*
**Prompt:** `second cut kick opponent stepping in trap setup taekwondo, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.4 — Pivot Setup [TEACH]
**Scene:** Kieryn **pivot foot plants** — chamber visible — inset hip rotation arrow.
**Prompt:** `turning kick chamber pivot foot diagram inset girl fighter, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH`

### 33.5 — Turning Head +5 [TEACH + ACTION] ★
**Scene:** **Turning head kick** — PSS flash — scoreboard **+5**.
**Dialogue:** Kieryn: "Head is three. Turning adds two."
**Prompt:** `girl landing turning head kick scoreboard plus five LED flash speed lines, STYLE: TOURNAMENT TEACH ACTION`

### 33.6 — Kieryn Smirk
**Scene:** Guard still up — smirk — MVP trash talk without words.
**Prompt:** `confident girl taekwondo fighter smirk guard up post score, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 33.7 — Round Win
**Scene:** Ref stops — Kieryn arm raised — team corner erupts.
**Prompt:** `referee raising girl fighter arm victory, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: HAIKYUU TOURNAMENT`

### 33.8 — Kian Whirlwind [ACTION]
**Scene:** Kian chaotic **360/tornado** combo — wins on activity — gam-jeom close call.
**Prompt:** `chaotic spinning taekwondo whirlwind combination wild card fighter, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.9 — Ariana Punch [TEACH + ACTION]
**Scene:** Ariana slips clinch — **punch +1** — decision win.
**Dialogue:** MV: "One point. That's all she needed."
**Prompt:** `close range punch scoring chest guard plus one decision win, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT TEACH ACTION`

### 33.10 — Logan Down on Points [ACTION]
**Scene:** Scoreboard — Logan trailing **4–6** — thirty seconds left — still charging.
**Dialogue:** Logan (internal): *One more step.*
**Prompt:** `scoreboard trailing short fighter still charging thirty seconds, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.11 — Logan Body Trade [ACTION]
**Scene:** Logan eats body kick — doesn't stop — answer cut — crowd roar.
**SFX:** *BEEP* *BEEP*
**Prompt:** `short fighter trading body kicks never stopping crowd, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.12 — Logan Loss Respect [HAIKYUU]
**Scene:** Ref raises opponent — Logan bows hard — opponent ruffles his head — grin.
**Dialogue:** Opponent: "You're scary, little man." · Logan: "Thanks!"
**Prompt:** `short fighter respectful loss bow opponent ruffling hair, STYLE: HAIKYUU TOURNAMENT`

### 33.13 — Kieryn Round Two [ACTION]
**Scene:** Second round — Maya answers cut — Kieryn feints — reads step-in again.
**Dialogue:** Kieryn (internal): *Same mistake. Different girl.*
**Prompt:** `girl fighter feint reading opponent step in trap, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.14 — Maya Gamble [ACTION]
**Scene:** Maya spins early — Kieryn ducks — illegal? — ref warning — close.
**Dialogue:** Ref: "Gam-jeom. Turning away."
**Prompt:** `girl ducking spin kick referee warning gam jeom close, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.15 — Kieryn Counter Body [ACTION]
**Scene:** Kieryn back kick trunk — clean — **+2** — guard never drops.
**SFX:** *BEEP*
**Prompt:** `girl back kick counter trunk sensor flash guard up, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.16 — Kian Tornado Miss [ACTION]
**Scene:** Kian **360** whiff — opponent counters — gam-jeom danger — chaos.
**Dialogue:** MV: "Plant! Plant!"
**Prompt:** `chaotic 360 kick miss opponent counter gam jeom danger, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.17 — Kian Ugly Win [ACTION]
**Scene:** Activity decision — Kian ahead on attempts — opponent frustrated — ref raises Kian.
**Dialogue:** Kian: "…I'll take it."
**Prompt:** `activity decision win chaotic fighter surprised relief, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT ACTION`

### 33.18 — Team Record Board [HAIKYUU]
**Scene:** Montage inset — TJ W · Kieryn W · Kian W · Ariana W · Logan L · Ethan L — MV tallies.
**Dialogue:** MV: "Four and two. First tournament. I'll take it."
**Prompt:** `team record montage four wins two losses proud coach chapter end, STYLE: HAIKYUU TOURNAMENT`

---

## Chapter 34 — Results & Poomsae Seed [PRESENT]

**Timeline:** **PRESENT** — West Regional tournament, **same day as Ch 25 loss**. Read **§34.0–34.10** Pineapple Fanta thread first, then **§34.1–34.3** results.

**Script:** [short-yuna-pineapple-fanta.md](../scripts/short-yuna-pineapple-fanta.md) § Arc 3

### 34.0 — Yuna Park Card [CARD]
**Scene:** Single intro card — poomsae athlete — not spar division.
**Overlay:** Yuna Park · 15 · 5'6" · 115 lbs · Poomsae Senior · 2nd Dan
**Prompt:** `single fighter profile card elegant girl poomsae athlete ponytail empty stats, STYLE: CARD TOURNAMENT`

### 34.4 — Vending Machine [PRESENT — post-loss hallway]
**Scene:** West Regional hallway — **one yellow can left**. Ethan half-geared — headphones around neck (first time Yuna sees them off).
**Prompt:** `tournament hallway vending machine one yellow soda can left teen in partial taekwondo gear, STYLE: TOURNAMENT`

### 34.5 — Procedure [PRESENT]
**Scene:** Coins *clink* — **Pineapple Fanta** drops — Ethan opens like **procedure**: sip, breathe, shoulders drop one inch.
**Dialogue:** Ethan (internal): *Open. Sip. Breathe.*
**Prompt:** `teen buying pineapple fanta soda vending machine ritual not happy not sad mask returning, STYLE: TOURNAMENT`

### 34.6 — Logan Interruption [HAIKYUU] [PRESENT]
**Scene:** Logan bounds over — too loud for hallway.
**Dialogue:** Logan: "Hyung! They have grape too—" · Ethan: "Not the same." · Logan: "It's both sugar!" · Ethan: *(walking)* "Pineapple."
**Prompt:** `cheerful little brother interrupting quiet teen soda vending machine comedy, STYLE: TOURNAMENT HAIKYUU`

### 34.7 — Yuna Watches [PRESENT]
**Scene:** Yuna from poomsae warm-up area — not teammate — just watching empty space where Ethan was.
**Dialogue:** Yuna (internal): *Never tried it. Mom only buys barley tea.*
**Prompt:** `poomsae girl athlete watching from distance curious quiet observer, STYLE: TOURNAMENT`

### 34.8 — Corner Store [PRESENT — evening, bus home]
**Scene:** Bus home — Yuna stops — buys Pineapple Fanta — too sweet, too yellow — doesn't throw away.
**Prompt:** `teen girl buying yellow pineapple soda corner store bus stop evening, STYLE: KWONS casual`

### 34.9 — Notebook Margin [PRESENT — later]
**Scene:** Poomsae notebook close-up — small handwriting.
**Dialogue (overlay):** *The quiet boy from sparring — pineapple soda after he loses. Remember.*
**Prompt:** `notebook margin handwritten note small letters poomsae athlete journal, STYLE: KWONS soft`

### 34.10 — Parking Lot Echo [PRESENT — end of tournament day]
**Scene:** Team loading van — Ethan holds empty **yellow can** — Yuna across lot with her unopened can — no wave yet — parallel strangers.
**Prompt:** `tournament parking lot team loading van teen with soda can girl watching from distance, STYLE: TOURNAMENT soft`

### 34.1 — Results Board [HAIKYUU]
**Scene:** Kwon's name mid-tier — team still proud. High fives.
**Prompt:** `team reading tournament results board proud not first place, STYLE: HAIKYUU`

### 34.2 — Premier Stare
**Scene:** Premier students in crowd — glare at Ethan. Ethan doesn't flinch.
**Prompt:** `rival school students glaring from crowd protagonist calm, white dobok full olympic kyorugi gear electronic chest hogu headgear forearm guards shin foot guards, STYLE: TOURNAMENT`

### 34.3 — Poomsae Ring [TEACH seed]
**Scene:** Adjacent ring — Yuna Park forms poomsae. Ethan glances — MV: "Different sport. Same discipline."
**Prompt:** `poomsae athlete performing forms adjacent sparring ring team watching, STYLE: TOURNAMENT TEACH`

### 34.11 — Logan Ice Pack [HAIKYUU]
**Scene:** Logan holds ice on shin — still grinning — lost but glowing.
**Dialogue:** Logan: "Did you see me not stop?" · Kieryn: "Hard to miss."
**Prompt:** `short teen ice pack shin still grinning post match, STYLE: HAIKYUU`

### 34.13 — Kieryn Victory Walk [HAIKYUU]
**Scene:** Kieryn walks like she owns venue — Maya nods respect across distance.
**Dialogue:** Maya: "Rematch." · Kieryn: "Bring it."
**Prompt:** `confident girl fighter victory walk rival nod respect, STYLE: HAIKYUU TOURNAMENT`

### 34.14 — Ariana Bandages Visible [KWONS]
**Scene:** Ariana rolls sleeve — bandages under gear — MV sees — says nothing — proud.
**Dialogue:** MV (internal): *Smart. Not broken.*
**Prompt:** `girl rolling sleeve bandages visible coach proud silent, STYLE: KWONS`

### 34.15 — Kian Can't Sit Still [HAIKYUU]
**Scene:** Kian bounces — ugly win energy — TJ pins him with one hand.
**Dialogue:** TJ: "You won. Act like it." · Kian: "I AM!"
**Prompt:** `hyperactive teen pinned by captain post win comedy, STYLE: HAIKYUU`

### 34.16 — Ethan & Results [KWONS]
**Scene:** Ethan finds his name — early exit bracket — doesn't look away.
**Dialogue:** Ethan (internal): *First tournament. Not last.*
**Prompt:** `teen reading own early loss on bracket board calm acceptance, STYLE: KWONS TOURNAMENT`

### 34.17 — Yuna Form Finish [TEACH]
**Scene:** Yuna lands final poomsae pose — judges bow — Ethan watches half-second too long.
**Dialogue:** MV: "Forms don't score contact. They score **you**."
**Prompt:** `poomsae athlete final pose judges bow sparring teen watching, STYLE: TOURNAMENT TEACH`

### 34.18 — Premier Walk-By [TOURNAMENT]
**Scene:** Premier group passes — Ttong Kim not present — students recognize Ethan — he holds gaze.
**Dialogue:** Premier student: "…Kwon's kid." · Ethan: *(nothing)*
**Prompt:** `premier students walking past protagonist holding gaze calm, STYLE: TOURNAMENT`

### 34.19 — Team Photo Real [HAIKYUU]
**Scene:** Official photographer — whole team — Ethan in back row — small smile — not performance.
**Dialogue:** Photographer: "Say—" · Logan: "KWON'S!" · *(flash)*
**Prompt:** `official team photo tournament whole team small smile flash, STYLE: HAIKYUU TOURNAMENT`

### 34.20 — Van Load Wide [TOURNAMENT]
**Scene:** Sunset — van doors close — arena behind — Yuna's bus two lanes over — parallel departure.
**Dialogue:** *(none — chapter end wide)*
**Prompt:** `sunset tournament van loading arena behind parallel buses chapter end, STYLE: TOURNAMENT soft`

---

## Chapter 35 — MV Evaluations [TEACH + HAIKYUU]

### 35.1 — Tape Review [TEACH]
**Scene:** MV tablet — freeze frame Ethan before spin. Team watches.
**Dialogue:** MV: "You saw the chamber. Your body remembered. Your feet didn't move."
**Prompt:** `coach tablet frame by frame technique review team watching, STYLE: KWONS TEACH`

### 35.2 — Scoring Context [TEACH]
**Scene:** MV draws on screen — where Ethan could counter for 2.
**Dialogue:** MV: "Next time — back kick on the spin. Two points. Their gamble becomes yours."
**Prompt:** `diagram counter back kick vs spinning attack instructional tablet, STYLE: KWONS TEACH`

### 35.3 — Ethan Speaks [HAIKYUU]
**Scene:** Ethan opens up — old school only ever said *go fight* — **board break hour**: wrong position, silent coaches, parents whisper *move board closer* — 1/5, would pass with zero, second despite highest score.
**Dialogue:** Ethan: "My old coach said go fight. That's all I ever got." · MV: "You would've passed with zero?" · Ethan: "…Yeah."
**Prompt:** `quiet teen opening up team listening support circle, STYLE: HAIKYUU KWONS`

### 35.4 — Next Time Together
**Dialogue:** MV: "Next time, we prepare together."
**Prompt:** `coach hand on shoulder promise warm dojang, STYLE: KWONS`

### 35.5 — Dojang Return [KWONS]
**Scene:** Team enters Kwon's night — shoes off — tired victory.
**Dialogue:** Logan: "Same time next week?" · MV: "Earlier."
**Prompt:** `team entering dojang night tired post tournament, STYLE: KWONS`

### 35.6 — Gear Unload [HAIKYUU]
**Scene:** Hogus stacked — smell joke — Kieryn opens window.
**Dialogue:** Kieryn: "That is biological warfare." · Kian: "Worth it."
**Prompt:** `stacked taekwondo gear smell joke window open comedy, STYLE: HAIKYUU`

### 35.7 — Tablet Setup [TEACH]
**Scene:** MV connects tablet to wall screen — Derek spin freeze frame loads.
**Dialogue:** MV: "Everyone watches. No phones."
**Prompt:** `coach tablet wall screen technique review setup, STYLE: KWONS TEACH`

### 35.8 — Chamber Frame [TEACH]
**Scene:** Freeze — Derek hip drop — red circle drawn — Ethan's feet circled wrong direction.
**Dialogue:** MV: "Chamber. You saw it. Here."
**Prompt:** `freeze frame spin chamber hip drop circled feet instructional, STYLE: KWONS TEACH`

### 35.9 — Back Kick Diagram [TEACH]
**Scene:** MV draws counter path — back kick into spin commit — +2 labeled.
**Dialogue:** MV: "They gamble. You collect."
**Prompt:** `diagram back kick counter spinning attack plus two points, STYLE: KWONS TEACH`

### 35.10 — Team Questions [HAIKYUU]
**Scene:** Kian raises hand — Logan raises both — Kieryn kicks Kian's chair.
**Dialogue:** Kian: "What if he fakes the fake?" · MV: "Then you feint the feint. Later."
**Prompt:** `team review session hands up comedy questions, STYLE: HAIKYUU TEACH`

### 35.11 — Logan Serious [HAIKYUU]
**Scene:** Logan rare quiet — asks about freezing — not joke.
**Dialogue:** Logan: "Does it still hurt in your head? Even when body doesn't?" · Ethan: "…Sometimes."
**Prompt:** `short teen serious question trauma team listening, STYLE: HAIKYUU KWONS`

### 35.12 — TJ Anchor [HAIKYUU]
**Scene:** TJ leans against wall — arms crossed — not speaking — present.
**Dialogue:** TJ: "We learn. We go back."
**Prompt:** `muscular teen anchor arms crossed quiet support, STYLE: HAIKYUU`

### 35.13 — Ethan Go Fight [PREMIER inset]
**Timeline:** **FLASH** on Ethan's word — **PRESENT** team circle.
**Scene:** Flash — Ttong Kim points — *Go fight* — snap present.
**Dialogue:** Ethan: "My old coach said **go fight**. That's all I ever got."
**Prompt:** `flashback coach pointing go fight snap to present team circle, STYLE: PREMIER inset HAIKYUU`

### 35.14 — Board Hour Speaks [HAIKYUU]
**Scene:** Ethan describes board — hour — wrong spot — silent coaches — voice rough not breaking.
**Dialogue:** Ethan: "Nobody moved it. Nobody told me where to stand."
**Prompt:** `teen describing trauma team circle listening rough voice, STYLE: HAIKYUU KWONS`

### 35.15 — Board Flash Inset [PREMIER]
**Scene:** Quick inset — clock — thud thud thud — parents whisper *move board closer*.
**Dialogue:** Parent (flash O.S.): "Why didn't they **adjust** anything?"
**Prompt:** `flash inset board break clock parents whispering, STYLE: PREMIER`

### 35.16 — Zero Passes [TEACH]
**Scene:** MV stops frame — looks at Ethan — not pity — math.
**Dialogue:** MV: "You would've passed with zero?" · Ethan: "…Yeah."
**Prompt:** `coach asking quiet teen would pass zero board teach moment, STYLE: KWONS TEACH`

### 35.17 — Hour Wasn't Board [KWONS]
**Scene:** MV sets tablet down — full attention — room silent.
**Dialogue:** MV: "Then the hour wasn't about the board."
**Prompt:** `coach setting tablet down profound silence team listening, STYLE: KWONS`

### 35.18 — Say Adjustment [TEACH]
**Scene:** MV demonstrates one step on mat — board imaginary — speaks out loud.
**Dialogue:** MV: "At Kwon's, we **say the adjustment out loud** — first time, not hour forty."
**Prompt:** `coach demonstrating step adjustment speaking out loud mat, STYLE: KWONS TEACH`

### 35.19 — Team Pledge [HAIKYUU]
**Scene:** Kieryn fist over heart — others follow — Ethan last — does it.
**Dialogue:** Kieryn: "Not there." · Team: "Not here."
**Prompt:** `team fist over heart pledge not here belonging, STYLE: HAIKYUU`

### 35.20 — Arc 3 Close [KWONS]
**Scene:** Wide — Kwon's empty except team + MV — tablet glow — Ethan exhales — first relief since Ch 25.
**Dialogue:** MV: "Next time, we prepare together." · Ethan: "…Yeah." · *(arc end)*
**Prompt:** `wide dojang team coach tablet glow teen exhaling relief arc end, STYLE: KWONS HAIKYUU`

---

**Arc 3 Total: 300 panels**

| Chapter | Title | Panels |
|---------|-------|--------|
| 21 | Equipment Fitting | 20 |
| 22 | Registration & Venue | 20 |
| 23 | Travel Banter | 20 |
| 24 | Ethan Gears Up | 20 |
| 25 | Round 1: The Kick | 20 |
| 26 | Premier Opening | 20 |
| 27 | School Grows | 20 |
| 28 | Board Break | 20 |
| 29 | Go Fight | 20 |
| 30 | Twenty Minute Scold | 20 |
| 31 | Back to Present | 20 |
| 32 | TJ's Match | 20 |
| 33 | Everyone Fights | 20 |
| 34 | Results & Poomsae Seed | 20 |
| 35 | MV Evaluations | 20 |
| **Total** | **Ch 21–35** | **300** |
